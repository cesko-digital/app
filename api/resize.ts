...
// Request validation
function prepareDataFromRequest(request: VercelRequest): ProcessedQueryParams {
...
function handleQueryParams(request: VercelRequest): ProcessedQueryParams {
  const { src: srcUrl, width: strWidth } = request.query

  if (typeof srcUrl !== 'string' || typeof strWidth !== 'string') {
    throw new ValidationError(
      'Arguments "src", "width" or "proof" are missing or invalid',
      400
    )
  }
