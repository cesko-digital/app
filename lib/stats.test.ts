...
function assertWriteCsvOutputError(
    lines: CsvLines | CsvGenerateLines
): void {
    const t = () => {
        const headers = ["label", "value-1", "value-3"];
    ...
    expect(t).toThrowError('Invalid count of a row values. The length must be same as head to be a valid CSV.');
}
