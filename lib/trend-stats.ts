import {buildCsvContent, CsvLine, CsvWriteLine} from "./stats";
...
export function getMonthColumn(date: Date): string {
    return romanize(date.getMonth() + 1) + '.' + date.getFullYear().toString().substring(2, 4)
}
...
export function buildTrendStats (
    ...
    function writeTrendData(trendValue: TrendValue) {
        const trendDate = new Date(trendValue.date.getFullYear(), trendValue.date.getMonth(), 1);

        // Filter out the data by the given year range
        if (options.fromYear !== null && trendDate.getFullYear() < options.fromYear) {
            return
        }
    ...
    return buildCsvContent(
        headers,
        function (write: CsvWriteLine) {
            const fromYearSetting = options.fromYear ? options.fromYear : earliestDate!.getFullYear();
            const toYearSetting = options.toYear ? options.toYear : latestDate!.getFullYear();

            const startDate = new Date(fromYearSetting, options.autoFill ? 0 : earliestDate!.getMonth(), 1)
            const endDate = new Date(toYearSetting, options.autoFill ? 11 : latestDate!.getMonth(), 1)
