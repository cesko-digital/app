...
export async function importObjects<E extends BaseEntity>({
  ...
  for (const updatedItem of itemsToUpdate) {
    ...
    const savedItem = await getObjectById(
      entity,
      apiKey,
      findPrevious(updatedItem)!.id,
    );
    const mergedItem = mergeEntities(savedItem, updatedItem, entity.mergeRules);
    const patch = diff(savedItem, mergedItem);
    if (Object.keys(patch).length > 0) {
      console.log(`Will update ${entity.singularName} ${savedItem.id}:`);
      console.log(JSON.stringify(patch, null, 2));
      try {
        if (!dryRun) {
          await updateObject(entity, apiKey, { ...patch, id: savedItem.id });
        }
      } catch (e) {
        console.error(
          `Failed to update ${entity.singularName} ${savedItem.id}:`,
        );
        console.error(e);
      }
    } else {
      console.log(
        `Skipping ${entity.singularName} ${savedItem.id}, no update needed.`,
      );
    }
  }
}
