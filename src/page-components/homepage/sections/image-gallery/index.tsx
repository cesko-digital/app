...
const ImageGallery: React.FC = () => {
  const ratioTotal = imageTreeTest.reduce((total, set) => total + set.ratio || 0, 0)
  return (
    <S.Container>
      <S.ContainerCentered>
      {imageTreeTest.map((set, key) => {
        set.ratio = 100 / ratioTotal * set.ratio || 100 / imageTreeTest.length // Set even size if ratio is not defined
        if (set.type === "row") {
          return <S.Row ratio={set.ratio}><Item {...set} key={key} /></S.Row>
        }
        if (set.type === "col") {
          return <S.Col ratio={set.ratio}><Item {...set} key={key} /></S.Col>
        }
        if (set.type === "img") {
